<%@page import="java.io.*,java.util.*"%><!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=0,minimal-ui"><title>Header presenti</title></head><body>
<pre><%

//File currentfile = new File(request.getRealPath(request.getServletPath()));
//File currentdir = currentfile.getParentFile();
//File[] m=new File(request.getRealPath(request.getServletPath())).getParentFile().listFiles();//files

File[] m=new File(request.getRealPath(request.getServletPath())).getParentFile().listFiles();//files

Arrays.sort(m);

File e=null;//file
String n="";
StringBuffer s = new StringBuffer("");//filename(s)

for(int i=0,l=m.length; i<l; i++){
 e=m[i];
 if(e.isFile()){
  n=e.getName().toLowerCase();
  if(n.endsWith(".jpg")){
   s.append(n.substring(0,n.length()-4)+"\r");
  };
 };
};
out.print(s);
%></pre>
</body></html>
